...
def run_cv(X: pd.DataFrame, y: pd.DataFrame,
    ...
    """Outer stratified CV; transforms fit only on train; evaluate on val."""
    metrics = []
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    for fold, (tr, va) in enumerate(skf.split(X, y['y_present_crc'])):
        ...
        print(f"[fold {fold}] risk1y: {risk1y}")
        print(f"[fold {fold}] risk3y: {risk3y}")
        print(f"[fold {fold}] riskLT: {riskLT}")
...
def _make_synthetic(n=500, seed=42):
    ...
    base_haz = np.clip(base_haz, 1e-6, None)  # avoid zero or negative hazards
    t_event = rng.exponential(1/base_haz)
...
def main_demo():
    ...
    output_csv = os.path.join(os.path.dirname(__file__), "synthetic_crc_data.csv")
    df.to_csv(output_csv, index=False)
    print(f"Synthetic dataset saved to {output_csv}")
    feature_cfg = {
        'clin': {'num':['age_years','bmi'], 'cat':['sex']},
        'germ': {'cols':['lynch_pathvar']},
